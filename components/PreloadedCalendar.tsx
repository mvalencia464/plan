...
const PreloadedCalendar: React.FC<PreloadedCalendarProps> = ({ year, events }) => {
  const COLUMNS = 37;
              ...
              return (
                <div 
                  ...
                  className={`aspect-square flex items-center justify-center relative ${
                    isActualDay ? '' : 'pointer-events-none'
                  }`}
                >
                  {isActualDay && (
                    <>
                      <div 
                        className={`w-[85%] h-[85%] flex items-center justify-center text-[9px] md:text-[11px] font-medium rounded-sm transition-all duration-300 ${
                          hasEvent 
                            ? 'bg-black text-white shadow-lg scale-110 z-10 font-bold' 
                            : isWeekend 
                              ? 'text-gray-500 bg-gray-300/30 hover:bg-gray-300/50'
                      ...
                      {hasEvent && (
                        <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-44 bg-white shadow-2xl rounded-md border border-gray-100 p-3 z-50 opacity-0 group-hover:opacity-100 pointer-events-auto transition-opacity">
                          {dayEvents.map((e, idx) => (
